...
function trials_2LoopBegin(trials_2LoopScheduler, snapshot) {
  return async function() {
    ...
    trials_2 = new data.MultiStairHandler({stairType:MultiStairHandler.StaircaseType.QUEST, 
      psychoJS: psychoJS,
      name: 'trials_2',
      varName: 'intensity',
      nTrials: 50.0,
...
function updateTiltRoutineBegin(snapshot) {
  return async function () {
    ...
    console.log(tiltSide*level)
    
    // keep track of which components have finished
    updateTiltComponents = [];
    
    for (const thisComponent of updateTiltComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function updateTilt2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of updateTilt2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
